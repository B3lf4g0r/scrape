...
class webScrapper
{
    ...
    public function setErrorLvl($level)
    {
        switch ($level) {
            case 'debug':
    ...
    public static function getData($url, $start = '', $end = '', $bot = false, $fields = '')
    {
        ...
        if (curl_errno($ch) > 0) {
            self::error('There was a cURL error: '.curl_error($ch).' on url: '.$url);
        } else {
            if (!empty($start)) {
    ...
    public static function updateTable($item)
    {
        ...
        mysql_query($_query) or self::error('Error on update: '.mysql_error().'. Query: <b>'.$_query.'</b>. URL: '.$item['item_url']);
        unset($_fields);
        unset($_query);
        unset($fields);
        unset($item);
